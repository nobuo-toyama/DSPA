...
curve(-x*log2(x)-(1-x)*log2(1-x), col="red", 
      main="Entropy for Different Proportions", 
      xlab = "x (proportion for class 1)", ylab = "Entropy", lwd=3)	
...
rf.fit <- randomForest(cd ~ . , data=qol_train[ , -40],
                       importance=TRUE, ntree=2000, mtry=26)	
varImpPlot(rf.fit, cex=0.5); print(rf.fit)	
	
rf.fit1 <- randomForest(cd ~ . , data=qol_train[ , -40],
                        importance=TRUE, ntree=2000, mtry=26)	
rf.fit2 <- randomForest(cd ~ . , data=qol_train[ , -40],
                        importance=TRUE, nodesize=5, ntree=5000, mtry=26)	
	
plot(rf.fit,log="x", 
     main="MSE Errors vs. Iterations: Three Models rf.fit, rf.fit1, rf.fit2")	
points(1:5000, rf.fit1$mse, col="red", type="l")	
points(1:5000, rf.fit2$mse, col="green", type="l")	
legend("topright", col=c("black", "red", "green"),
       lwd=c(2, 2, 2), legend=c("rf.fit", "rf.fit1", "rf.fit2"))	
