---
title: "10 Black Box Machine-Learning Methods: Neural Networks and Support Vector Machines"
author: "nobuo"
date: "2021/4/25"
...
plot_ly() %>%	
  add_trace(x = x, y = y1/10, type="scatter", mode="markers", name="Linear") %>%	
  add_trace(x = x, y = y2/5, type="scatter", mode="markers", name="Saturated Linear") %>%	
  add_trace(x = x, y = y3, type="scatter", mode="markers", name="Hyperbolic Tangent") %>%	
...
library(neuralnet)
google_model <- neuralnet(RealEstate ~ Unemployment + Rental + 
                            Mortgage + Jobs + Investing + 
                            DJI_Index + StdDJI, data=google_train)	
plot(google_model)
```
...
```{r}
google_model2 <- neuralnet(RealEstate ~ Unemployment + Rental + 
                             Mortgage + Jobs + Investing + 
                             DJI_Index + StdDJI, data=google_train, hidden = 4)	
plot(google_model2)
```
...
```{r}
google_model2 <- neuralnet(RealEstate ~ Unemployment + Rental + Mortgage + 
                             Jobs + Investing + DJI_Index + StdDJI,
                           data=google_train, hidden = c(4,3,3))	
google_pred2 <- compute(google_model2, google_test[, c(1:2, 4:8)])	
pred_results2 <- google_pred2$net.result	
cor(pred_results2, google_test$RealEstate)	
```
...
```{r}
nn_single = neuralnet(High + Median + Low ~ 
                        Unemployment + Rental + Mortgage + Jobs +
                        Investing + DJI_Index + StdDJI,
                        data = train,	
                      hidden=4,
                      linear.output=FALSE,
...
Below is one example where the 2D data doesn't appear as linearly separable in its native 2D.	
...
We can see that only few cases of may be classified as Iris versicolor. The species of the majority of the flowers are all correctly identified.	
...
set.seed(2020)	
gammas = exp(-5:5)	
tune_g = tune.svm(Species ~ ., kernel = "radial", data = iris_train,
