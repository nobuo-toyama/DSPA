...
the optimum by by taking into account the history of the prior gradients. Although there are many alternative formulations, the general search direction is computed by $\nu_k=\begin{cases} -\nabla f(x_k) & \text{if k=0} \\ -\nabla f(x_k)+\gamma_k\nu_{k-1} & \text{if k>0} \end{cases}$, where one option for the coefficients is the Fletcher–Reeves estimates $\gamma_k =\frac{||\nabla f(x_k)||^2}{||\nabla f(x_{k-1})||^2}$. And the learning-rate is a number that satisfies $\begin{cases} f(x_k)-f(x_k +a_k \nu_k) \ge -\delta a_k (\nabla f(x_{k}))' \nu_k \\ ||(\nabla f(x_{k}+a_k\nu_k))'||\leq -\sigma (\nabla f(x_{k}))' \nu_k   \end{cases}$, for some pair of positive real values $ 0<\delta<\sigma<1$.
...
The Newton’s method is often an improvement over first-order gradient descent optimization methods as it also utilizes the second-order Hessian to estimate the search descent direction and the learning rate as follows:	
...
$$f(x_1, x_2) = (x_1-3)^2 + (x_2+4)^2+x_1 x_2.$$
```{r}
...
### Compute and Plot the unique 3D point of the intersecting 3 planes	
library(geometry)	
library(pracma)	
...
plot_ly() %>%	
  ...
  layout(title="Hyperplane constraits in 3D", legend = list(orientation='h'),	
...
plot_ly() %>%   	
  ...
  layout(title="Graphical Representation of a Quadratic Function Optimizaiton") %>%	
  hide_colorbar()
```
...
plot_ly() %>%   	
  ...
  layout(title="Quadratic Function Optimizaiton Gradient Descent Paths") %>%	
  hide_colorbar()
```
